...
export default class ParseImage {
  ...
  constructor(filename: string, data: Buffer, contentType?: string) {
    ...
    if (!type || !type?.startsWith("image/")) {
      throw new Parse.Error(
        ...
        `${filename} has not valid mime type.`
      );
    }
  }
  ...
  async process<Sharp>(
    block: (proc: sharp.Sharp) => sharp.Sharp
  ): Promise<sharp.Sharp>;
  async process(
    block: (proc: sharp.Sharp) => Buffer | sharp.Sharp
  ): Promise<Buffer | sharp.Sharp> {
    return block(sharp(this.data).withMetadata());
  }

  async edit(
    block: (proc: sharp.Sharp) => Promise<Buffer>
  ): Promise<Parse.File> {
  ...
  async rotate(angle?: number): Promise<Parse.File> {
    const bufferedData = await this.process((proc) =>
      proc.rotate(angle).toBuffer()
    );
    return new Parse.File(this.filename, {
      base64: bufferedData.toString("base64"),
    });
  }

  async resize(width?: number, height?: number): Promise<Parse.File> {
    const bufferedData = await this.process((proc) =>
      proc.resize(width, height).toBuffer()
    );
    return new Parse.File(this.filename, {
      base64: bufferedData.toString("base64"),
    });
  ...
  async scale(width = 1, height = 1): Promise<Parse.File> {
    const bufferedData = await this.process((proc) =>
      proc.metadata().then((metadata) => {
        return proc
          .resize({
            width: Math.round(metadata.width * width),
            height: Math.round(metadata.height * height),
          })
          .toBuffer();
      })
    );
